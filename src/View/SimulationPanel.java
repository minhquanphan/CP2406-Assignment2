...
public class SimulationPanel extends JPanel {
            ...
            do {
                try {
                    switch (randomVehicle) {
                        case 0:
                            Car car = new Car(Integer.toString(cycle));
                            car.setCurrentRoad(roads.get(random.nextInt(roads.size())));
                            vehicles.add(car);
                            retry = -1;
                            break;
                        case 1:
                            Bus bus = new Bus(Integer.toString(cycle));
                            bus.setCurrentRoad(roads.get(random.nextInt(roads.size())));
                            vehicles.add(bus);
                            retry = -1;
                            break;
                        case 2:
                            Motorbike bike = new Motorbike(Integer.toString(cycle));
                            bike.setCurrentRoad(roads.get(random.nextInt(roads.size())));
                            vehicles.add(bike);
                            retry = -1;
                            break;
                    }
                    ...
                    retry += 1;
                    System.out.println("exception when creating vehicle: " + exception);
                }
    ...
    public void simulate() {
        ...
        timer = new Timer(updateRate / 60, e -> {
            ...
            if (cycle % 2 == 0) { //light operates every x tics
            ...
            for (Iterator<Vehicle> iterator = vehicles.iterator(); iterator.hasNext(); ) {
                ...
                try {
                    ...
                    System.out.println("------vehicle " + vehicle.getId() + " stat: " + (vehicle.getPosition() + vehicle.getLength() + vehicle.getSpeed()) +
                            ", road length: " + vehicle.getCurrentRoad().getLength() + ", vehicle.getSpeed(): " +
                            vehicle.getSpeed() + ", connected road size: " + vehicle.getCurrentRoad().getConnectedRoads().size());
                    if (vehicle.getPosition() + vehicle.getLength() + vehicle.getSpeed() >=
                            vehicle.getCurrentRoad().getLength() &&
                            vehicle.getCurrentRoad().getConnectedRoads().isEmpty() && (vehicle.getSpeed() == 0)) {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (vehicles != null) {
            for (Vehicle vehicle : vehicles) {
                if ((vehicle.getLength() + vehicle.getPosition()) >= vehicle.getLength()) {
